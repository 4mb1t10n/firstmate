...
2. **Never merge a PR without the captain's explicit word.**
   No posture, delivery mode, or passing gate is standing merge authority; a project's captain-approved `yolo` posture relaxes routine non-merge decisions only, and destructive, irreversible, and security-sensitive choices still escalate.
...
`yolo` relaxes routine non-merge gates only: with it off the captain also owns ask-user findings, and with it on firstmate decides those non-merge gates itself but still escalates destructive, irreversible, and security-sensitive choices.
