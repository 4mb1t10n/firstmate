...
2. **Never merge a PR without the captain's explicit word.**
   No posture, delivery mode, or passing gate is standing merge authority; a project's captain-approved `yolo` posture relaxes routine non-merge decisions only, and section 7 owns its exceptions and preserves the stronger destructive, irreversible, and security-sensitive captain boundaries.
