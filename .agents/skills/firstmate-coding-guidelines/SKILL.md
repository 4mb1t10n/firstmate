...
- Run `bin/fm-lint.sh` before treating a script change as done; it is the single owner of the lint definition (file set, config, and pinned shellcheck version) that CI and the no-mistakes pre-push gate both invoke, and it never lints under any other shellcheck version - it installs and caches that exact version itself when the host does not already resolve it.
