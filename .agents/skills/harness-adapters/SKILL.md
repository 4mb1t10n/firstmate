...
The companion watcher extension checks the secondmate Codex reserve at `before_agent_start` with the live provider and model, which keeps queued follow-ups and post-launch model changes inside the same turn-start boundary.
