...
| `fm-startup-network.sh`  | Run session start's network checks and reconciliation tick off its blocking path in a bounded detached worker, and publish the result inline or as a wake |
