...
8. Ordinary wake: when the background task completes with `signal:`, `stale:`, `check:`, `reconcile:`, or `heartbeat`, drain queued wakes, then start exactly one fresh background task before running other fleet commands to handle the wake.
