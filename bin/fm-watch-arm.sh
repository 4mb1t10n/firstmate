...
watch_output_has_wake() {
  local out=$1
  grep -Eq '^(signal:|stale:|check:|reconcile:|heartbeat($|:))' "$out" 2>/dev/null
}

watch_output_reason_type() {
  local out=$1 line
  line=$(grep -E '^(signal:|stale:|check:|reconcile:|heartbeat($|:))' "$out" 2>/dev/null | head -1 || true)
  case "$line" in
    signal:*) printf 'actionable-signal' ;;
    stale:*) printf 'actionable-stale' ;;
    check:*) printf 'actionable-check' ;;
    reconcile:*) printf 'actionable-reconcile' ;;
