...
# PENDING REMOVAL. First Mate never merges: AGENTS.md hard rule 2 admits no
# exception, so no gate result authorizes this call. Until it is removed this
# script must not be invoked; the gate above is still correct and is what an
# evaluate-and-report path should reuse (docs/reconciliation-heartbeat.md).
#
# Rationale to preserve for whoever removes it: every task PR merge lands through
# bin/fm-pr-merge.sh (AGENTS.md section 7), which records the canonical pr= and
# the forge's pr_head= before merging so bin/fm-teardown.sh can still verify
# landed work after the squash. Calling gh-axi directly here merged around that
# recording and left a merged task with no PR reference to verify against, so any
# future merge path must keep that recording rather than drop back to gh-axi.
