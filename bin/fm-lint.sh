...
# Print the version a binary reports, or nothing when it is absent or broken.
probe_shellcheck_version() {
  "$1" --version 2>/dev/null | awk '/^version:/ {print $2; exit}'
}

# Bootstrap the pin unless PATH already resolves exactly it.
...
resolved=$(probe_shellcheck_version "$SHELLCHECK_BIN")
