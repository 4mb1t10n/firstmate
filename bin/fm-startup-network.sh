...
# auth status`, the reconciliation heartbeat, the secondmate liveness and convergence sweeps (11 sequential,
...
#                             delivery, fleet sync, reconciliation), one per
#                             secondmate for the remote-touching steps (id and
#                             host), and one per project clone. Published for a
#                             timed-out or failed run too, where a partial record is the answer.
...
phase_label() {  # <phases>
  case "$1" in
    ...
    probe,sweeps,reconciliation) printf 'GitHub authentication, dead-secondmate relaunch, secondmate convergence, pending handoff delivery, project clone refresh with its drift reporting, and reconciliation heartbeat' ;;
...
cmd_start() {  # <locked> <harvest-pid>
  ...
  if [ "$locked" = 1 ]; then
    phases=probe,sweeps
    [ "${FM_RECONCILE_SESSION_START:-1}" = 0 ] || phases=probe,sweeps,reconciliation
  fi
...
cmd_run() {  # <locked> <lock-pid> <generation>
  ...
  local timings stage_started elapsed remaining reconcile_budget reconcile_rc reconcile_started
  mkdir -p "$STATE" 2>/dev/null || return 1
  started=$(now)
  budget=$(stage_budget)
  phases=probe
...
EOF
  ...
  # The base branch's forced session-start reconciliation belongs in this same
  # deferred stage: it performs GitHub inventory and writes durable state, so
  # running it in the session hook would restore the network stall this worker
  # exists to prevent. Give it only the aggregate stage budget still remaining.
  if [ "$sweep_locked" -eq 1 ] && [ "${FM_RECONCILE_SESSION_START:-1}" != 0 ]; then
    elapsed=$(( $(now) - started ))
    remaining=$((budget - elapsed))
    reconcile_budget=${FM_RECONCILE_SESSION_START_TIMEOUT:-60}
    case "$reconcile_budget" in ''|*[!0-9]*|0) reconcile_budget=60 ;; esac
    [ "$reconcile_budget" -le "$remaining" ] || reconcile_budget=$remaining
    printf '\nRECONCILIATION\n' >> "$out"
    if [ "$reconcile_budget" -gt 0 ]; then
      reconcile_rc=0
      reconcile_started=$(fm_timing_now_ms)
      fm_run_timed "$reconcile_budget" "$SCRIPT_DIR/fm-reconcile.sh" --tick --force >>"$out" 2>&1 \
        || reconcile_rc=$?
      fm_timing_record phase reconciliation "$reconcile_started"
      if [ "$reconcile_rc" -eq 124 ]; then
        printf 'RECONCILIATION_ERROR: tick exceeded %ss and was stopped; GitHub inventory is stale. Repair the forge connection and rerun bin/fm-reconcile.sh --tick --force before dispatch.\n' "$reconcile_budget" >> "$out"
      ...
      printf 'RECONCILIATION_ERROR: the deferred network stage exhausted its %ss aggregate bound before the tick; GitHub inventory is stale. Rerun bin/fm-reconcile.sh --tick --force before dispatch.\n' "$budget" >> "$out"
    fi
  fi
