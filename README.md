...
- **Reconciliation heartbeat** - a periodic GitHub-to-fleet control loop treats every open issue in your configured repositories as authorized work, reserving each one with an `in-progress` label lease before a crew starts, so silence between crew events cannot strand work.
...
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, runtime backend selection, optional X mode and its X and Discord setup steps, the files you set, and harness support.
- [docs/reconciliation-heartbeat.md](docs/reconciliation-heartbeat.md) - the periodic GitHub-to-fleet control loop: cadence selection, issue leases, durable acknowledgement, process ownership, and the feature-to-`stg` merge-readiness report.
